...
// CommitInfo holds essential details for a single Git commit.
...
func GitCommitInit(repoPath string, allBranch bool) {
	...
	gitCommit := GitCommit{
		ID:                          uuid.New(),
		RepoPath:                    repoPath,
		GitCommits:                  make([]CommitInfo, 0, 1),
		GitCommitsDAG:               make([][]string, 0, 1),
		ErrorLog:                    []error{},
...
func (gc *GitCommit) GetLatestGitCommitInfoAndDAG(updateChannel chan string) {
	gc.ID = uuid.New()
	gc.MU.Lock()
	gc.Status = INITIALIZING
	currentID := gc.ID
	gc.GitCommitsDAG = make([][]string, 0, 1)
	gc.GitCommits = make([]CommitInfo, 0, 1)
	gc.getCommitLogInfo(currentID, updateChannel)
	gc.Status = DONE
	gc.MU.Unlock()
}

func (gc *GitCommit) getCommitLogInfo(currentID uuid.UUID, updateChannel chan string) {
	...
	// preallocate space
	gc.GitCommits = make([]CommitInfo, 0)
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, recordSeparator) {
			...
			// Create and append the CommitInfo struct.
			gc.GitCommits = append(gc.GitCommits, CommitInfo{
			...
			updateChannel <- GIT_COMMIT_UPDATE
		}
	}
